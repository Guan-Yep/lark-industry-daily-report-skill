...
2. **极佳的阅读体验**：先输出可读的 Markdown 草稿，再由脚本宽容解析并利用 `lark-whiteboard` 自动渲染三列排版画板，同时生成小红书风格交互式卡片。
...
3. **抓取与分析**：并发抓取最新资讯，调用 LLM 严格分类提炼并输出 `draft_report.md`。
4. **知识沉淀**：脚本从 `draft_report.md` 自动解析生成 `summary.md` 和 `board.json`，并在知识库中产出《YYYY-MM-DD 行业日报》。
5. **卡片推送**：给你的飞书发交互式卡片，并在发送前自动守护启动监听进程，避免按钮回调离线。
...
### 4. 回调监听机制（默认守护）
`python scripts/send_card.py` 在发送卡片前会自动守护启动 `listen_feedback.py`，通常无需手工额外启动监听。
...
### 5. 日报核心产物
每次流程会产出以下文件，便于排查与复用：
- `all_news.json`：抓取并过滤后的新闻原始集合
- `draft_report.md`：LLM 生成的结构化 Markdown 草稿
- `summary.md`：标准化后的正文（用于写入飞书文档）
- `board.json` / `board_openapi.json`：白板 DSL 与 OpenAPI 格式
- `message_card.json`：交互式消息卡片内容
...
* **状态存储**：偏好与规则沉淀在飞书云端（`lark-sheets` / `lark-docs`），本地仅保留轻量状态文件。
